...
The native `startCapture(20s)` loop is the real driver while the child is in **another** app (RN JS timers are throttled in the background), so its interval is set to **20s** (`intervalMs` prop, floored by `RISK_INTERVAL_LOW_MS`). The **JS** `setTimeout` periodic chain adds app-aware adaptivity while SafeGuard is foreground (interval **0** clears that timer). Native frames are debounced/OCR-limited. Note: the native interval is fixed for the session, so background high-risk still scans at ~20s (the 10s tier only applies when the JS timer is live); app-switch + 5s follow-up still give fast first detection.
...
| **Periodic capture** | **10 / 15 / 20 s** by risk (+ app category); native loop fixed at **20s** | How often the screen is re-scanned while staying in an app |
...
| **Vision / processing watchdog** | **~25 s** | Force-release hung frame processing (may not fire while JS is frozen in background) |
