...
| **Phase B** | 15 – 27 Sept 2026 (branch `phase-b`, not yet merged) | In progress | **Adult-site blocking (Chrome only)** — accessibility-service address-bar watcher detects a known domain (StevenBlack static list, ~76.8k entries) or a vision-confirmed adult page (raw NSFW ≥ 0.7) and blacklists it; Back + a full-screen block overlay for a listed domain, Back-only for a first-detection or an OCR-only false positive (no list write); on the block screen's OK, Chrome is relaunched onto a fresh blank tab (`OverlayWindowHelper.BlockDismissGate`, ordering load-bearing for the `BAL_ALLOW_NON_APP_VISIBLE_WINDOW` exemption). Parent-side: `blocked_domains` / `browser_block_incidents` tables, dev-only unblock endpoint, read-only dashboard panels, device-side backfill/offline-queue/reconcile sync. **Open finding:** whether the enforcement's Back press actually moves the page depends on how fast it fires, not on the tab's navigation history — a fast static-list match (~130 ms) measured as a no-op, a slower vision-confirmed detection (seconds after page load) measured as moving the page to `about:blank`, on the same zero-history tab shape; unresolved on purpose, see CLAUDE.md for the full measurement. Full detail, JVM/Jest/backend test counts, and known limits (incognito, non-Chrome browsers, drawn/animated content, thumbnail-grid pages under-scoring on raw vision) are in `CLAUDE.md`'s Phase B section, not duplicated here. |
